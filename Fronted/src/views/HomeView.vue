...
onMounted(async () => {
  try {
    // 1. On vérifie immédiatement si le backend a une session pour nous
    await UserService.getCurrentUser();

    // 2. Si ça réussit, on ne reste pas ici, on va au dashboard
    // Note : On utilise /app/dashboard pour correspondre à ton nouveau routeur
    router.push('/app/dashboard');
  } catch (error) {
    // 3. Si erreur (401), c'est qu'on doit vraiment se connecter
